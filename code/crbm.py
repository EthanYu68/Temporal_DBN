...
class CRBM(object):
    ...
    def __init__(self, input=None, input_history=None,
                n_visible=None, n_hidden=None, delay=None, freq=None,forward=None,
        ...
        self.forward = forward # number of forward visible units
        ...
        if W is None:
            ...
            initial_W = np.asarray(0.01 * numpy_rng.randn(n_visible*forward,
                                                          n_hidden),
        ...
        if vbias is None:
            ...
            vbias = theano.shared(value=np.zeros(n_visible*forward,
    ...
    def sample_h_given_v(self, v0_sample, v_history):
        ...
        h1_sample = self.theano_rng.binomial(size=h1_mean.shape, n=1,
                                             p=h1_mean,
                                             dtype=theano.config.floatX)
        #h1_sample = self.theano_rng.normal(size=h1_mean.shape,avg=h1_mean,std=0.01,dtype=theano.config.floatX)
    ...
    def sample_v_given_h(self, h0_sample, v_history):
        ...
        #                                   n=1, p=v1_mean,
         #                                   dtype = theano.config.floatX)
        v1_sample = self.theano_rng.normal(size=v1_mean.shape, avg=v1_mean, std=0.01, dtype=theano.config.floatX)
    ...
    def train(self, learning_rate=None, training_epochs=None,
                 dataset=None, seqlen= None, batch_size=None, full_seq=None):
        ...
        n_train_batches = (dataset.get_value(borrow=True).shape[0]-((self.delay+self.forward)*self.freq)*len(seqlen)) // batch_size
        #n_train_batches = (dataset.shape[0] - (self.delay * self.freq) * len(seqlen)) / batch_size
        # valid starting indices
        datasetindex = []
        last = 0
        if full_seq == True:
            for s in seqlen:
                datasetindex += range(last + (self.delay*self.freq), last + s-self.forward)
                last += s
            permindex = np.array(datasetindex)
        else:
            n_train_batches = int(np.sum(seqlen)/(self.delay+1)/batch_size)
            for s in seqlen:
                datasetindex += range(last + (self.delay * self.freq), last + s-self.forward,(self.delay * self.freq+1))
                last += s
            permindex = np.array(datasetindex)
        ...
        cost, updates = self.get_cost_updates(lr=learning_rate, k=5)
        ...
        train_crbm = theano.function(
                ...
                cost,
                updates=updates,
                givens={self.input: dataset[index].reshape((batch_size,self.forward*self.n_visible)),
        ...
        for epoch in range(training_epochs):
            ...
            for batch_index in range(n_train_batches):
                ...
                this_cost = train_crbm(visi_idx.ravel(), hist_idx.ravel())
...
def create_train_crbm(learning_rate=3e-3, training_epochs=1000,
             dataset=None, seqlen= None, batch_size=10,full_seq =False,forward=3,
             n_hidden=50, delay=6, freq=1):
    """
    ...
    crbm = CRBM(input=x, input_history=x_history, n_visible=n_dim, n_hidden=n_hidden, delay=delay, freq=freq,forward=forward)

    crbm.train(learning_rate=learning_rate,full_seq=full_seq,training_epochs=training_epochs, dataset=dataset, seqlen= seqlen, batch_size=batch_size)
    return crbm
