...
class LOGISTIC_RBM(object):
    ...
    def __init__(self, numpy_rng=None, theano_rng=None,
                 n_input=2, n_hidden=50, n_label=3):
        """
        ...
        self.rbm_layer = RBM(numpy_rng=numpy_rng, theano_rng=theano_rng,
                               input=self.x,
                               n_visible=n_input, n_hidden=n_hidden)

        self.params.append(self.rbm_layer.W)
        self.params.append(self.rbm_layer.hbias)

        # We now need to add a logistic layer on top of the MLP
        input_logistic = T.nnet.sigmoid(T.dot(self.x, self.rbm_layer.W)+ self.rbm_layer.hbias)
    ...
    def build_finetune_functions(self, dataset_train, labelset_train, seqlen_train,
        ...
        n_valid_batches = (dataset_validation.get_value(borrow=True).shape[0])/batch_size
        ...
        def valid_score():
            ...
            for batch_index in range(n_valid_batches):
                ...
                valid_score_out.append(valid_score_i(data_idx))
        ...
        def test_score():
            ...
            for batch_index in range(n_test_batches):
                data_idx = permindex[batch_index * batch_size:(batch_index + 1) * batch_size]
                test_score_out.append(test_score_i(data_idx))
    ...
    def recognize_dataset(self, dataset_test = None, seqlen=None, batch_size =100) :
        ...
        n_test_batches = (dataset_test.get_value(borrow=True).shape[0]) / batch_size
        n_test_batches = np.int64(n_test_batches)
        ...
        input_log = T.nnet.sigmoid(T.dot(self.x, self.rbm_layer.W) + self.rbm_layer.hbias)
        ...
        datasetindex = range(0, dataset_test.get_value(borrow=True).shape[0])
        permindex = np.array(datasetindex)
        ...
        for batch_index in range(n_test_batches):
            data_idx = permindex[batch_index * batch_size : (batch_index + 1) * batch_size]
            pred, prob = print_prediction(data_idx)
            predict.append(pred)
...
def create_train_LogisticCrbm(
                    ...
                    pretrain_lr=1e-3, k=2, training_epochs=100,
                    ...
                    batch_size=100, number_hidden_rbm=50, n_label=3, retrain_rbm = True, log_rbm=None):
    ...
    #########################
    if (retrain_rbm):
        log_rbm = LOGISTIC_RBM(numpy_rng=numpy_rng, n_input=n_dim, n_label = n_label, n_hidden=number_hidden_rbm)
        print('... pre-training the model')
        cost_rbm = log_rbm.rbm_layer.train(learning_rate=pretrain_lr, training_epochs=pretraining_epochs, dataset=dataset_train, seqlen= seqlen_train, batch_size=batch_size)
    ...
    n_train_batches = (dataset_train.shape[0]) / batch_size

    # get the training, validation and testing function for the model
    print('... getting the finetuning functions')
    train_fn, validate_model, test_model = log_rbm.build_finetune_functions(
    ...
    n_train_batches = int(n_train_batches.eval())
    ...
    while (epoch < training_epochs) and (not done_looping): #TODO supprimer le not done_looping pour forcer le bouclage?
        ...
        for minibatch_index in range(n_train_batches):
            ...
            minibatch_avg_cost = train_fn(data_idx) #training model : fine tune phase
    ...
    return log_rbm, train_fn,validate_model, test_model
