...
class RBM(object):
    ...
    def __init__(self, input=None, n_visible=21, n_hidden=30,
        ...
        if theano_rng is None:
            theano_rng = MRG_RandomStreams(numpy_rng.randint(2 ** 30)) #, use_cuda = True)
    ...
    def free_energy(self, v_sample):
        ...
        vbias_term = 0.5 *T.sum(T.sqrt(v_sample - self.vbias))
...
def create_train_rbm(learning_rate=1e-3, training_epochs=200,
             dataset=None, seqlen = None, batch_size=10, n_hidden=30):
    """
    ...
    n_train_batches = int(dataset.get_value(borrow=True).shape[0] / batch_size )#number of minibatch
